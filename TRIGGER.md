# Trigger.dev v4 — full vendored reference

> **This file is the AUTO-VENDORED Trigger.dev v4 reference, moved verbatim out of `CLAUDE.md`
> (2026-07-26).** It was ~1,430 lines — 93% of `CLAUDE.md` — being loaded into every session, while
> this codebase uses about a dozen of the APIs it documents. `CLAUDE.md` now carries only the rules
> that are load-bearing here and points at this file.
>
> **Nothing was edited.** The five `<!-- TRIGGER.DEV … START/END -->` blocks below are byte-identical
> to what was in `CLAUDE.md`, markers included.
>
> ⚠️ **The markers mean a tool generated this.** If `trigger.dev init` (or whatever docs installer put
> them there) runs again, it may RE-INJECT these blocks into `CLAUDE.md`. If you ever see the Trigger
> reference back in `CLAUDE.md`, that's what happened — move it here again rather than deleting it.
>
> **When to read this file:** you need a Trigger API this project doesn't already use — batch
> triggering, debouncing, tags, metadata, queues/concurrency, machine presets, logging/tracing, hidden
> tasks, build extensions beyond `additionalFiles`, or Realtime/React hooks. For anything current,
> prefer the **context7 MCP** (the global instruction says to use it for docs) — a vendored copy goes
> stale; context7 does not.
>
> **What this project actually uses** is summarised in `CLAUDE.md` → "Trigger.dev v4 — as used in this
> project". Start there.
