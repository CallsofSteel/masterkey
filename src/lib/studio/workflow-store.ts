...
export type SkillNodeType = "purpose" | "service" | "instruction" | "decision" | "input" | "output" | "loop";
...
export type WorkflowNodeData = {
  ...
  // ── Service node fields ──
  endpoint?: EndpointData;    // lightweight display shape for the canvas node renderer (price/summary/method)
  notes?: string;             // User's notes about this service
  // ── Masterkey registry-backed service node (spec §3.3) ──
  // The canvas working model carries these; the serializer (§7.5) maps them to §1.1 BundleNode.data:
  // `service` → BundleNode.data.endpoint (the embedded snapshot), the rest pass through 1:1.
  ...
  inputMap?: Record<string, string>; // schema field -> template ref ("{{nodeId.path}}") or literal
  service?: BundleService;     // full registry snapshot (embedded detail, re-resolved fresh at compile)
  ...
  // ── Loop node fields (spec §7.4) ──
  overRef?: string;           // template ref to the collection to iterate
  until?: string;             // plain-English stop condition
  bodyNodeIds?: string[];     // node ids forming the loop body
...
// Service browser modal (spec §7.3) — a visual, category-grouped picker (mirrors the catalog). Set this
// from anywhere (the node palette, the "+" menu, a blank service node's "Browse" button); WorkflowCanvas
// renders the single modal and, on select, either CREATES a new service node ("create"; optionally
// connected to sourceId at position) or SETS the service on an existing node ("set").
export type ServiceBrowserRequest =
  | { mode: "create"; sourceId?: string; sourceHandle?: string | null; position?: { x: number; y: number } }
  | { mode: "set"; nodeId: string };
export const serviceBrowserAtom = atom<ServiceBrowserRequest | null>(null);

// "Change type" on a node's config panel → open the node palette to pick a new kind for THIS node (instead
// of the deprecated legacy block picker). Holds the node id being re-typed (null = normal add mode).
export const changeTypeNodeIdAtom = atom<string | null>(null);

// ─── Test run (spec §9.2 / §10) — the in-builder E2E test surface ───────────────────────────────────
// The "Test" button opens a transcript DRAWER over the canvas (graph stays editable, build-assist chat
// beside it) instead of navigating to /run/[id]. It collects the bundle's inputs[] up-front (§9.2), starts
// a normal durable run, and shows the live transcript (reuses RunView). `null` = drawer closed.
export const testDrawerOpenAtom = atom<boolean>(false);
// The active test run's id (set once the run starts), so the drawer + chat can reference it.
export const activeTestRunIdAtom = atom<string | null>(null);
// The DEBUG LOOP hand-off: a runId the build-assist chat should review. The ChatBar watches this — when
// set, it auto-sends a "review this test run" message (the same brain that built the bundle reads the
// run's transcript/blockers via get_run_result and fixes/improves the graph), then clears it.
export const assistReviewRunIdAtom = atom<string | null>(null);
