...
} from "@/lib/studio/legacy-plugins";
import { Panel } from "../ai-elements/panel";
import { WorkflowIcon } from "@/components/ui/workflow-icon";
import { AccountMenu } from "@/app/account-menu";
import { useTheme } from "@/lib/theme";
// Flow's wallet-status + user-menu chrome dropped (spec §2.4/§7.7): money is invisible in Masterkey and the
// app has its own dashboard nav. The toolbar's right cluster is left empty for Masterkey chrome later.
...
function useWorkflowActions(state: ReturnType<typeof useWorkflowState>) {
  ...
  const handleDownload = async () => {
    ...
    toast.info("Exporting SKILL.md…");

    try {
      // Masterkey export (spec §5.4/§11) — a SKILL.md takeaway for the user's own external agent, NOT a
      // Claude-Code plugin zip. Replaces Flow's /api/workflows/[id]/export.
      const response = await fetch(`/api/studio/bundles/${currentWorkflowId}/export?format=skill`, {
        method: "POST",
      ...
      a.href = url;
      a.download = `${workflowName.toLowerCase().replace(/[^a-z0-9]/g, "-")}.skill.md`;
  ...
  const handleDuplicateWorkflow = async () => {
    ...
    try {
      ...
      window.location.href = `/bundles/${copy.id}/edit`;
    } catch (error) {
...
function ToolbarActions({
  ...
  return (
    <>
      ...
      <div className="flex lg:hidden">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-56">
            <DropdownMenuItem onClick={() => setServiceBrowser({ mode: "create" })}>
      ...
      {/* Duplicate (no Save button — changes autosave silently) */}
      <ButtonGroup orientation="horizontal">
        <DuplicateButton handleDuplicate={actions.handleDuplicateWorkflow} state={state} />
      </ButtonGroup>

      {/* Export takeaway (§11.2) + run the bundle inside Masterkey (whole-bundle test run) */}
      <ExportBundleButton state={state} />
      <RunBundleButton state={state} />
    </>
  );
}

// Theme toggle — a standard icon button (matches the other toolbar icons), separate from the account menu.
function ThemeToggleButton() {
  const { resolvedTheme, setTheme } = useTheme();
  const isDark = resolvedTheme === "dark";
  return (
    <Button
      className="border bg-card shadow-sm hover:bg-black/5 dark:hover:bg-white/5"
  ...
  return (
    <>
      ...
      <AlertDialog onOpenChange={setOpen} open={open}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Duplicate this bundle?</AlertDialogTitle>
            <AlertDialogDescription>
              This creates a separate copy (&ldquo;{state.workflowName || "this bundle"} (copy)&rdquo;) that you
              can edit independently. The original is unchanged. You&apos;ll be taken to the copy.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
// Run Button — runs the bundle inside Masterkey (the brain executes the recipe; the platform pays). This
// is the whole-bundle test run (spec §7.7/§10): save the latest graph, resolve the slug, POST /api/runs
// with "/<slug>", and open the live run. Replaces Flow's Claude-Code "Create Skill" (/api/chat) seam.
function RunBundleButton({ state }: { state: ReturnType<typeof useWorkflowState> }) {
  const setTestDrawerOpen = useSetAtom(testDrawerOpenAtom);

  // Opens the in-builder test drawer (§9.2/§10.1) — it saves the latest graph, collects inputs up-front,
  // and runs the compiled bundle, all without leaving the canvas. (Was: navigate to /run/[id].)
  return (
    <Button
      className="gap-1.5 border bg-green-600 px-3 text-xs font-medium text-white hover:bg-green-700 disabled:opacity-50"
      disabled={!state.currentWorkflowId || state.nodes.length === 0 || state.isGenerating}
      onClick={() => setTestDrawerOpen(true)}
      size="sm"
      title="Test this bundle end-to-end inside Masterkey (live transcript opens beside the canvas)"
      variant="default"
    >
...
// Export the CURRENT canvas (spec §11.2): SKILL.md takeaway, bundle .json, or copy markdown — all hit
// /api/studio/bundles/[id]/export with the live nodes/edges (so unsaved edits export too).
function ExportBundleButton({ state }: { state: ReturnType<typeof useWorkflowState> }) {
  const [copied, setCopied] = useState(false);

  const call = async (format: "skill" | "json"): Promise<string> => {
    const res = await fetch(`/api/studio/bundles/${state.currentWorkflowId}/export?format=${format}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nodes: state.nodes, edges: state.edges, name: state.workflowName }),
    });
    if (!res.ok) throw new Error("export failed");
    return format === "json" ? JSON.stringify(await res.json(), null, 2) : await res.text();
  };

  const download = async (format: "skill" | "json") => {
    try {
      const text = await call(format);
      const slug = (state.workflowName || "bundle").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "bundle";
      const blob = new Blob([text], { type: format === "json" ? "application/json" : "text/markdown" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = format === "json" ? `${slug}.bundle.json` : `${slug}.skill.md`;
      a.click();
      URL.revokeObjectURL(url);
    } catch {
  ...
  const copyMd = async () => {
    try {
      await navigator.clipboard.writeText(await call("skill"));
      setCopied(true);
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          className="gap-1.5 border px-3 text-xs font-medium"
          disabled={!state.currentWorkflowId || state.nodes.length === 0 || state.isGenerating}
          size="sm"
          title="Export this bundle as a takeaway (your own agent pays per call over x402)"
          variant="secondary"
        >
          {copied ? <Check className="size-3.5 text-emerald-500" /> : <Download className="size-3.5" />} Export
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => download("skill")}>
      ...
      <div className="pointer-events-auto absolute top-4 right-4 z-10">
        <div className="flex flex-col-reverse items-end gap-2 lg:flex-row lg:items-center">
          <ToolbarActions
            ...
            workflowId={workflowId}
          />
          {/* Masterkey chrome: a standard theme-toggle icon (separate from the account section) + the
              account menu (navigate away, sign out). Changes autosave, so there's no Save button. */}
          <ThemeToggleButton />
          <div className="flex items-center rounded-lg border border-border bg-card/90 px-1 shadow-sm backdrop-blur">
            <AccountMenu />
          </div>
        </div>
      </div>
